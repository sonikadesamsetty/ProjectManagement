...
@Component({
  selector: 'app-story',
  standalone: true,
  imports: [FormsModule, ReactiveFormsModule, MatIconModule, DatePipe],
...
export class StoryComponent implements OnInit, OnChanges {
  ...
  //get the value of comments to empty its value after saving
      ...
      this.storyService.getStories().subscribe(stories => {
        ...
        if (this.storyId > 0) {
          ...
          this.storyService.getStoryById(this.storyId).subscribe(storyData => {
            ...
            //this.discussionComments = storyData.comments;
            console.log("Discussion Comments: ", this.story.comments);
            this.discussionComments = this.story.comments;
            this.discussionComments.sort((a, b) => {
  ...
  async saveChanges() {
    ...
    if (this.storyId == 0) {
      //create a new story
      const newStory = new Story(0, this.storyForm.value.title || '', this.storyForm.value.description || '', this.storyForm.value.status || '', this.storyForm.value.assignedTo || '', this.storyForm.value.priority || '', this.storyForm.value.capacity || 0, this.storyForm.value.dor || '', this.storyForm.value.acceptanceCriteria || '', this.storyForm.value.sprintId, this.itemType || 'story');
      ...
      const updatedStory = new Story(this.storyId || 0, this.storyForm.value.title || this.story?.title, this.storyForm.value.description || this.story?.description, this.storyForm.value.status || this.story?.status, this.storyForm.value.assignedTo || this.story?.assignedTo, this.storyForm.value.priority || this.story?.priority, this.storyForm.value.capacity || this.story?.capacity, this.storyForm.value.dor || this.story?.dor, this.storyForm.value.acceptanceCriteria || this.story?.acceptanceCriteria, this.storyForm.value.sprintId || this.story?.sprintId, this.itemType || this.story?.itemType || 'story');
      updatedStory.modifiedBy = this.userEmail;
  ...
  addComment(commentText: any) {
    console.log("addComment called");
    console.log(commentText);
    if (commentText.trim().length === 0) {
      return;
    }
    const comment: Comment = new Comment(0, commentText, this.userEmail, new Date(), this.userEmail, new Date(), 0, this.storyId || 0);
    this.storyService.addStoryDiscussion(comment).subscribe((res: any) => {
      console.log("Discussion updated: ", res);
      this.getAllComments();
      this.discussionText = '';

      //empty the discussion input box
      //emptying discussion text won't empty the comments.value, so we need to do it manually
      //use commentsContainer and empty its value
      this.commentsContainer.nativeElement.value = '';
  ...
  getAllComments() {
    ...
    console.log("Fetching comments for story id: ", this.storyId);
    this.storyService.getCommentsByStoryId(this.storyId || 0).subscribe((comments: Comment[]) => {
