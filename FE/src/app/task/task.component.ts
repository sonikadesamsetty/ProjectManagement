...
@Component({
  selector: 'app-task',
  standalone: true,
  imports: [NgFor, FormsModule, ReactiveFormsModule, MatIconModule, DatePipe],
...
export class TaskComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.taskId == 0) {
      console.log("Creating new task, skipping data fetch");
      
    } else {
      console.log("Fetching task data for task ID: ", this.taskId);
    this.assignee = this.members.find((member: { id: number; }) => member.id == Number(this.task?.assignedTo))?.name || '';
    this.taskService.getTask(this.taskId).subscribe((task) => {
      ...
      this.discussionComments = this.task.comments;
  ...
  addComment(commentText: any) {
      ...
      const comment: Comment = new Comment(0, commentText, this.userEmail, new Date(), this.userEmail, new Date(),  this.taskId || 0,0);
      this.taskService.addTaskDiscussion(comment).subscribe((res: any) => {
    ...
    getAllComments() {
      ...
      console.log("Fetching comments for story id: ", this.taskId);
      this.taskService.getCommentsByTaskId(0 || this.taskId).subscribe((comments: Comment[]) => {
